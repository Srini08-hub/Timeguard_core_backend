...
from src.control.agents.excel_nodes.load_sheet import excel_node
...
from src.control.agents.nodes.hybridpdf_node import hybrid_pdf_node
...
from src.control.agents.pdf_nodes.load_pdf import pdf_node
...
from src.control.agents.scanned_pdf_nodes.load_pages import scanned_pdf_node
...
async def build_email_graph() -> CompiledStateGraph[TimeguardState]:
    ...
    graph.add_node("hybrid_pdf_node", hybrid_pdf_node)
    ...
    graph.add_node("excel_node", excel_node)
    ...
    graph.add_edge("excel_node", "next_sheet")
    ...
    graph.add_edge("pdf_node", "next_page")
    ...
    graph.add_node("scanned_pdf_node", scanned_pdf_node)
    ...
    graph.add_conditional_edges(
        ...
        {
            "pdf_node": "pdf_node",
            "scanned_pdf_node": "scanned_pdf_node",
            "hybrid_pdf_node": "hybrid_pdf_node",
            "increment_attachment_node": "increment_attachment_node",
        },
    )

    graph.add_edge("image_node", "increment_attachment_node")
    graph.add_edge("hybrid_pdf_node", "increment_attachment_node")
