"""Node to update attachment status after scanned PDF classification."""
...
async def update_scanned_pdf_attachment_status(
    state: TimeguardState,
    config: RunnableConfig,
) -> TimeguardState:
    """Update attachment status based on scanned PDF classification result."""
    db_session = get_db_session(config)
    attachment_repository = AttachmentRepository(db_session)

    classification = state.get("scanned_pdf_classification")
    ...
    if attachment_db_id:
        ...
        if attachment is not None:
            ...
            logger.info(
                "Updated attachment %s to status %s based on scanned PDF classification %s",
                attachment_db_id,
                attachment_status,
                classification,
            )
