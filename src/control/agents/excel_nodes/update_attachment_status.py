"""Node to update attachment status after Excel classification."""
...
async def update_excel_attachment_status(
    state: TimeguardState,
    config: RunnableConfig,
) -> TimeguardState:
    """Update attachment status based on Excel classification result."""
    db_session = get_db_session(config)
    attachment_repository = AttachmentRepository(db_session)

    classification = state.get("excel_classification")
    ...
    if attachment_db_id:
        ...
        if attachment is not None:
            ...
            logger.info(
                "Updated attachment %s to status %s based on Excel classification %s",
                attachment_db_id,
                attachment_status,
                classification,
            )
